...
def main(number_of_patients):
    ...
    decisionVariables(solver, number_nurses, number_patiens, number_days, number_hours, d, x, y, s)

    # Add objective function to the solver
    objectiveFunction(solver, d, number_patiens)

    # Add master constraints to the solver
    masterConstraints(solver, d, x, y, s, number_nurses, number_patiens, number_days, number_hours, q, Q, frequency,et,lt,sd)
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        sol_s = {key: s[key].solution_value() for key in s}

        # Print solution
        printSolution(solver, sol_d, sol_x, sol_y,sol_s, number_nurses, number_patiens, number_days,number_hours, df, df_n, q, frequency)
    else:
